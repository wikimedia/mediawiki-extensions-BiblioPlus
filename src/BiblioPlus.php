...
class BiblioPlus {
	...
	/**
	 * @var array Management of the citation indices (order in which they appear in the text).
	 */
	public $citations = [];
	 ...
	 * @param int $key The index of the desired element.
	 ...
	 *
	 * @param int $key The index of the citation to return or create.
	 * @param bool $create Indicates whether or not a new reference number should be created.
	 * @return int The reference number, or -1 if one was not created.
	 */
	 ...
	 *
	 * @param array|string|int $a The input variable.
	 ...
	 * @return string|int A random element from the array.
	 */
	...
	function parseBiblio( $list ) {
		...
		return [ 'entries' => $result, 'pmids' => $pmids, 'isbns' => $isbns ];
	}
	 ...
	 * @param array $pdata Parser data.
	 * @param bool $render If true, citation will be created and returned; if false, returns empty string.
	 ...
	 * @param array $pdata Parser data.
	 * @param bool $force If true, references will be listed even if not cited in text;
	...
	function renderBiblio( $input, $pdata, $force ) {
		...
		foreach ( $entries as $ref ) {
			...
			if ( $pmid !== null ) {
				// PubMed result
				$pmlink = $this->htmlInterLink( $this->pubMedUrl( [ $pmid ] ),
					$this->smallCaps( ' PubMed ID:' ) . "$pmid", "PubMed ID: $pmid" );
				$hmlink = $this->htmlInterLink( $this->hubMedUrl( [ $pmid ] ),
